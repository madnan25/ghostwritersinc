...
/**
 * POST /api/series/:id/cancel — cancel a series.
 *
 * Allowed from any non-terminal status (planning, active, paused).
 * Marks all remaining (non-done) brief stubs as cancelled.
 */
export async function POST(
  ...
  if (briefsError) {
    console.error('[series/cancel] DB error cancelling briefs:', briefsError)
  ...
  if (error) {
    console.error('[series/cancel] DB error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
