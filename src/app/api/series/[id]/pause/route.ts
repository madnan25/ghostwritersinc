...
/**
 * POST /api/series/:id/pause — pause a series.
 *
 * Transitions: planning → paused, active → paused.
 * Freezes remaining (non-done, non-cancelled) brief stubs.
 */
export async function POST(
  ...
  const allowedFromStatuses = ['planning', 'active']
  if (!allowedFromStatuses.includes(series.status)) {
    return NextResponse.json(
      { error: `Cannot pause a series with status: ${series.status}` },
      { status: 422 }
    )
  }
  ...
  if (error) {
    console.error('[series/pause] DB error:', error)
