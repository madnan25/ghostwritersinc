...
export async function GET(
  ...
  const limited = await rateLimit(getAgentRateLimitKey(auth, 'strategy-documents-read'), { maxRequests: 60 })
...
export async function PATCH(
  ...
  const limited = await rateLimit(getAgentRateLimitKey(auth, 'strategy-documents-write'), { maxRequests: 20 })
  if (limited) return limited
