...
export async function GET(request: NextRequest) {
  ...
  const limited = await rateLimit(getAgentRateLimitKey(auth, 'strategy-config-read'), { maxRequests: 60 })
...
export async function PUT(request: NextRequest) {
  ...
  const limited = await rateLimit(getAgentRateLimitKey(auth, 'strategy-config-write'), { maxRequests: 10 })
  if (limited) return limited
