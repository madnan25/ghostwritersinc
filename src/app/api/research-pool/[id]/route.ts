...
export async function GET(
  ...
  const limited = await rateLimit(getAgentRateLimitKey(auth, 'research-pool-read'), { maxRequests: 60 })
...
export async function PATCH(
  ...
  const limited = await rateLimit(getAgentRateLimitKey(auth, 'research-pool-write'), { maxRequests: 10 })
  if (limited) return limited
